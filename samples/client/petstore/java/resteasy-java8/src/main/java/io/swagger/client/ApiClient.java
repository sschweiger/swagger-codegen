      ...
      for (Entry<String, Object> param : formParams.entrySet()) {
        if (param.getValue() instanceof File) {
          ...
          try (InputStream stream = new FileInputStream(file)) {
            multiPart.addFormData("file", stream, MediaType.APPLICATION_OCTET_STREAM_TYPE);
          } catch (IOException e) {
            throw new ApiException("error uploading file");
          }
        } else {
          multiPart.addFormData(param.getKey(), parameterToString(param.getValue()), MediaType.APPLICATION_OCTET_STREAM_TYPE);
        }
      }
